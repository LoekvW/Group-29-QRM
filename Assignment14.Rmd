...
```{r}
library(ggplot2)
movies1$profit_millions <- (movies1$revenue - movies1$budget) / 1e6
quantile(movies1$profit_millions * 1e6, na.rm = TRUE)

ggplot(movies1, aes(x = "", y = profit_millions)) +
  geom_boxplot(fill = "lightblue", color = "darkblue", outlier.color = "red") +
  labs(
    title = "Distribution of Movie Profits",
    x = "Movies",
    y = "Profit (in Millions USD)"
  ) +
  theme_minimal()
